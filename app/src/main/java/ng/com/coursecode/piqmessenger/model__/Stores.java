...
public class Stores {
    ...
    public static final String PLAY_LIKE = "play_sound_like";
    ...
    Context context;
    private String apiKey="@fgfgxfbf67667@_gjvg";
    ...
    public String getUsername() {
        return Prefs.getString(Profile.USERS_NAME, "").trim();
    }
    ...
    public void handleError(String error, Context context, ServerError serverError) {
        handleError(error, context, serverError, false);
    }

    public void handleError(String error, Context context, ServerError serverError, boolean b) {
        int retu=Stores.getErrorInt(error);
        switch (retu){
            case R.string.empty_response:
                serverError.onEmptyArray();
                break;
            case R.string.relogin:
                if(!b){//b=dnt go home
                ...
                serverError.onShowOtherResult(context.getString(R.string.relogin));
                break;
            default:
                serverError.onShowOtherResult(context.getString(retu));
                break;
        }
    }

    private static int getErrorInt(String error) {
        int retu=R.string.error_fetchin_item;
        switch(error){
            case "6479":
                retu=R.string.empty_response;
                break;
            case "3474":
                retu=R.string.relogin;
                break;
            case "1474":
                retu=(R.string.no_username);
                break;
            case "2474":
                retu=(R.string.no_pass);
                break;
            case "5474":
                retu= (R.string.inv_app_id);
                break;
            case "5475":
                retu=(R.string.no_auth);
                break;
            case "6474":
                retu= (R.string.error_edit_prof);
                break;
            case "6475":
                retu= (R.string.undef_req);
                break;
            case "6476":
                retu= (R.string.inv_req);
                break;
            case "6477":
                retu= (R.string.inv_file);
                break;
            case "6478":
                retu= (R.string.missing_var);
                break;
            case "6480":
                retu= R.string.inv_user;
                break;
            case "6481":
                retu= (R.string.inv_email);
                break;
            case "6483":
                retu= (R.string.inv_user_or_email);
                break;
            case "6485":
                retu= (R.string.int_error);
                break;
            default:
                retu= (R.string.error_fetchin_item);
                break;
        }
        return retu;
    }
    ...
    public String getFirebaseStore(String profileStore) {
        ...
        return profileStore+dateTime1.getMonth()+"/"+dateTime.getWeekIndex()+"/"+ dateTime.getDay()+"/"+dateTime1.getMilliseconds(TimeZone.getDefault())+"/";
    }
    ...
    public static int getLoader(Context context) {
        int prf=parseInt(Prefs.getString("loader_type", "0"));
        int thumbnail=R.drawable.loader1;
        if(prf==1){
            thumbnail=R.drawable.loader;
        }
        return thumbnail;
    }

    public static ServerError getServerError(final Context context){
